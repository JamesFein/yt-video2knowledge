...
[[ -f "$REPO_ROOT/data/youtube-oauth-token.json" ]] || fail "Missing data/youtube-oauth-token.json; run uv run yt-video2knowledge digest --youtube-auth first"
[[ -d "$REPO_ROOT/data/chrome-automation-profile" ]] || fail "Missing managed Chrome profile; run uv run yt-video2knowledge digest --seed-from-current-profile first"
