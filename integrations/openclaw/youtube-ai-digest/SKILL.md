...
- `src/yt_video2knowledge/digest/`: production digest domain modules.
- `src/yt_video2knowledge/site/`: Knowledge Site implementation.
- `tests/`: mirrors the production modules; `experiments/` stays outside production imports.
- `integrations/openclaw/`: this skill's thin OpenClaw adapter; `data/`: local run state.

Dependency direction is `cli → digest.run / site.sync`, `digest.run → digest domain modules`, and `site.sync → digest.manifest`.
...
```bash
uv run yt-video2knowledge digest --help
```
...
Recover a missing or damaged manifest from existing per-video artifacts without downloading or calling a model:

```bash
uv run yt-video2knowledge recover-manifest --target-date YYYY-MM-DD
```

Sync completed artifacts into the Knowledge Site explicitly:
