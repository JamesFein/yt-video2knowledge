...
> **归档状态**：该方案已经实施，本文只保留当时的问题、权衡和验收思路，不是当前待办。
> 文中的“当前”“改造前”和行号均指方案形成时；现在的 Markdown parser 是
> `src/yt_video2knowledge/site/markdown.py`，结构化 `body_segments` 路径已有对应测试。

> 目标：让视频详情页（`src/yt_video2knowledge/site/templates/video.html`）左侧 Summary 阅读区，
> 在**窄屏单列阅读**场景下更舒服。配套原型见 `prototype.html`。

## 涉及文件

- `src/yt_video2knowledge/site/templates/video.html` —— Summary block 的结构
- `src/yt_video2knowledge/site/static/site.css` —— 阅读区样式
- `src/yt_video2knowledge/site/markdown.py` —— 正文从 markdown 转纯文本（仅批次 B 涉及）

## 原型怎么看

浏览器打开 `prototype.html`，顶部两组切换：

- **并排对比 / 仅改造前 / 仅改造后**
- **手机宽 / 桌面宽**

「改造前」还原的是归档时的 `site.css` 规则；「改造后」是当时计划的目标效果。它们都不是当前线上页面的逐像素基线。
...
| # | 问题 | 位置 | 严重度 |
|---|------|------|--------|
| ① | **正文内部层级被压平**：小标题、加粗、列表被抹成纯文本，靠空行硬撑，无法扫读 | 归档时的 `markdown_blocks.py`；当前对应 `site/markdown.py` | 🔴 高 |
...
- **必须补测试**：`tests/` 下为新解析/渲染加用例（子标题、加粗、有序/无序列表、空 body、代码块边界），跑 `uv run python3 -m unittest discover -s tests -t .`。
...
- [ ] 批次 B：新增解析/渲染单测全绿
- [ ] 批次 B：选中写入的文本与屏幕所见一致
- [ ] 回归：`uv run python3 -m unittest discover -s tests -t .`
- [ ] 同步：如改了 block 数据结构，确认 `yt_video2knowledge.site.sync` 不受影响

---

## 备注

- 原型 `prototype.html` 与本文档均为历史方案产物，**不参与线上渲染**。
